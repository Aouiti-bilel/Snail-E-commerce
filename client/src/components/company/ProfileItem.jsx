...
const ProfileItem = ({ auth, addComment, addLike, removeLike, products: { products, loading }, profileById : { likes, imageUrl, location,  name, website, phone, user, social, _id, comments }}) => {    
          ...
          <a href={social.instagram} target='_blank' rel='noopener noreferrer'>
            ...
            { 
              isAuthenticated &&<div className='review'>
               <form  onSubmit = {e => onSubmit(e)}>
                         <textarea
                           ...
                           />
                          <input type='submit' value='Write a Review' className='Write-a-Review'/>
                 </form>
             </div>
            }
             <div className='comment-container'>
             {
              ...
              <div className='box-I'><img src={ comment.avatar} alt={imageUrl} style={{ paddingRight:'10px', borderRadius:'50%', width:'50px', height:'50px'}} /></div>
              <div className='box-II'> {comment.text}<Moment fromNow>{comment.date}</Moment></div> 
              <div className='box-III'> <Moment fromNow>{comment.date}</Moment></div>  
                 </div>
               } ) }
             </div>
             </div>
...
export default connect(mapStateToProps, { addComment, addLike, removeLike })(ProfileItem) 